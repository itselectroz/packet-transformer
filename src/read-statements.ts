...
const createReadType = (
  typeData: TypeData
): ts.Expression | VectorStatement => {
  ...
  switch (type) {
    ...
    case "constant": {
      ...
      switch (constant.type) {
        case "boolean": {
          return constant.value ? factory.createTrue() : factory.createFalse();
        }
        case "string": {
    ...
    case "vector": {
      ...
      statements.push(
        factory.createForStatement(
          ...
          factory.createPostfixIncrement(uniqueIndexName),
    ...
    case "custom": {
      ...
      if (typeof className != "string") {
        throw new Error(
          `Something went wrong. Expected className to be type 'string' got '${typeof className}'`
        );
      }
    ...
    case "nbits": {
      ...
      if (typeof numBits != "number") {
        throw new Error(
          `Something went wrong. Expected className to be type 'number' got '${typeof numBits}'`
        );
      }
...
*/
export function createReadFunction(
  className: string,
  expressions: ts.Statement[]
  ...
  return factory.createMethodDeclaration(
    ...
    factory.createBlock(
      [
        ...
        ...expressions,
        factory.createReturnStatement(factory.createIdentifier("packet")),
      ],
      true
    )
  );

...
import {
  createReadTypeAssignment,
  createReadFunction,
} from "./read-statements";
    ...
    case ts.SyntaxKind.TypeReference:
      ...
      if (type == "vector") {
        ...
        if (!typeArguments || typeArguments.length != 2) {
          throw SyntaxError(
            `Not enough arguments for vector type. Expected 2 got ${
        ...
        if (["constant"].includes(valueType.type)) {
          throw SyntaxError(
            `Invalid value type in vector. Got ${valueType.type}`
          );
        }

        if (
          ["custom", "vector", "boolean", "float"].includes(lengthType.type)
        ) {
          throw SyntaxError(
            `Invalid length type in vector. Got ${lengthType.type}`
          );
        }
        ...
        if (!typeArguments || typeArguments.length != 1) {
          throw SyntaxError(
            `Not enough arguments for custom type. Expected 1 got ${
              typeArguments?.length || 0
            }`
          );
        }
        ...
        if (typeArgument.kind != ts.SyntaxKind.TypeReference) {
          throw SyntaxError(
            `Expected TypeReference in custom type. Got ${typeArgument.kind}`
          );
        }
        ...
        if (!typeArguments || typeArguments.length != 1) {
          throw SyntaxError(
            `Not enough arguments for nbits type. Expected 1 got ${
              typeArguments?.length || 0
            }`
          );
        }
        ...
        if (typeArgument.kind != ts.SyntaxKind.LiteralType) {
          throw SyntaxError(
            `Expected LiteralType in nbits type. Got ${typeArgument.kind}`
          );
        }
        ...
        if (literal.kind != ts.SyntaxKind.NumericLiteral) {
          throw SyntaxError(
            `Expected NumericLiteral in nbits type. Got ${typeArgument.kind}`
          );
        }
    ...
    const rootNodeVisitor = (node: ts.Node): ts.Node => {
      ...
      switch (node.kind) {
        ...
        case ts.SyntaxKind.ClassDeclaration:
          ...
          const writeStatements: ts.Statement[] = classData.properties.reduce(
          ...
          const readFunction = createReadFunction(
            classData.className,
            readStatements
          );
